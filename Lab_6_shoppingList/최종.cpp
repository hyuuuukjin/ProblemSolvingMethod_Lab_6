...
int new_mem(struct customer* custo);  //신규 회원 가입용, 포인트 적립하는 함수
int ext_mem(struct customer* cus);  //기존 회원용, 포인트 적립하는 함수
...
int main()
{
	...
	while(1)
	{
		...
		{
			printf("\n회원번호를 입력하세요(전화번호 끝 4자리): ");
			scanf_s("%d", &mem_num);

			//입력 받은 회원 번호와 일치하는 회원 정보 찾기
			for (int i = 0; i < 3; i++)
			{
				if (mem_num == cus[i].num)  //입력 받은 회원 번호 == 등록된 회원 정보
				{
					ext_mem(&cus[i]);  //기존 회원 포인트 적립
					printf("--------------------------\n");
					printf("이름: %s님 \n", cus[i].name);
					printf("회원번호: %d \n", cus[i].num);
					printf("주소: %s \n", cus[i].address);
